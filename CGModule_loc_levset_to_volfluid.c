		  ...
		  int nrhs, const mxArray *prhs[])
{
  ...
  // JK modificaiton. it is said that mxGetpr is not recommended in general 
  vf = mxGetPr(plhs[0]);
  //vf = mxGetDoubles(plhs[0]); 
  ...
  for (k=0;k<Npix;k++){ /* Loop over pixels. */
    ...
    // This is just to define periodicity !! 
    E = n*wrap(xk+1,n) + yk;
    W = n*wrap(xk-1,n) + yk;
    N = n*xk + wrap(yk+1,n);
    S = n*xk + wrap(yk-1,n);
        
    // mx and my are gradient 
    mx = 0.5*(Z[E]-Z[W]);
    my = 0.5*(Z[N]-Z[S]);
