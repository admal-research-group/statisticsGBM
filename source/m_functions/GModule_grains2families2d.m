function [families,famgrains] = GModule_grains2families2d(grains,R,dims,WORKSPACE,work_x, work_y, work_bx, work_by, work_candidate_x, work_candidate_y)
% Group grains into families in a way that grains in a same family are separated by at least R pixels.  
%
% :param grains: grains 
% :param R: the minmum distance grains within the same family 
% :return familes: 
% :return famgrains: 
...
while N>sum(remaininggrains>0) % Loop over families.
    ...
    for k=1:N % Loop over the grains.
        ...
        if (intersects(k)==0 && remaininggrains(k)==0) % If grain k is unallocated
            ...
            [x2,y2] = CGModule_pgrow3(int32(x),int32(y),R,WORKSPACE,...
