function [grains,cluster] = EModule_initialdoubleVoronoidata2d(N,dims,Rgrains,rn, FactorNc)
% Generates initial condition with N grains on a grid of size dims using double Voronoi layers
% Each grain belong to one cluster which are described by a coarser Voronoid seed
%
% :param N: the number of total grains in the system 
% :param dim: the size of grid 
% :param Rgrain: the width of periodic growth (window size)  
% :param rn: the random seed number. For example, just 1,2,3...etc
% :param FactorNc: the approximated number of grains in each cluster 
% :return grains: grains
% :return cluster: 1d array of indices for the cluster number of grains 
...
for k=1:N % Loop over the grains.
    ...
    [x2,y2] = CGModule_pgrow3(int32(x),int32(y),Rgrains,W,...
